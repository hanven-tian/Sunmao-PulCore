...
test('HTTP vertical slice exposes foundation plugins and portal CRUD', async (context) => {
  ...
  const health = await fetch(`${base}/health`).then((response) => response.json()); assert.equal(health.name, 'PulCore'); assert.equal(health.plugins.length, 10); assert.equal(health.plugins.every((plugin) => plugin.status === 'enabled'), true);

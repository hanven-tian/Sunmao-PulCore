...
async function createFoundation() { const core = new PulCore(); const plugins = [accessControlPlugin, portalManagerPlugin, workflowPlugin, auditLogPlugin, ...foundationSuitePlugins]; for (const plugin of plugins) core.plugins.install(plugin); for (const plugin of plugins) await core.plugins.enable(plugin.name); return core; }
...
test('foundation plugins install models and expose capabilities', async () => { const core = await createFoundation(); assert.equal(core.plugins.list().length, 10); assert.deepEqual(core.models.list().map((item) => item.name), ['user', 'role', 'portal', 'workflow', 'audit_log', 'data_source', 'ui_schema', 'file_asset', 'notification', 'data_job', 'scheduled_task']); assert.equal(core.plugins.list().every((item) => item.status === 'enabled'), true); assert.equal(core.plugins.list().find((item) => item.name === 'file-manager').capabilities.includes('file.upload'), true); });

...
import { foundationSuitePlugins } from '../src/plugins/foundation-suite.js';
import { officialMatrixPlugins } from '../src/plugins/official-matrix.js';

async function createFoundation() { const core = new PulCore(); const plugins = [accessControlPlugin, portalManagerPlugin, workflowPlugin, auditLogPlugin, ...foundationSuitePlugins, ...officialMatrixPlugins]; for (const plugin of plugins) core.plugins.install(plugin); for (const plugin of plugins) await core.plugins.enable(plugin.name); return core; }
...
test('full plugin matrix installs and exposes capabilities', async () => { const core = await createFoundation(); const list = core.plugins.list(); assert.equal(list.length, 44); assert.deepEqual(core.models.list().map((item) => item.name), ['user', 'role', 'portal', 'workflow', 'audit_log', 'data_source', 'ui_schema', 'file_asset', 'notification', 'data_job', 'scheduled_task']); assert.equal(list.every((item) => item.status === 'enabled'), true); assert.equal(list.every((item) => item.license === 'free'), true); assert.equal(list.find((item) => item.name === 'file-manager').capabilities.includes('file.upload'), true); assert.equal(list.find((item) => item.name === 'auth-saml').capabilities.includes('auth.saml'), true); assert.equal(list.find((item) => item.name === 'ai-data-qa').upstreamTier, 'enterprise'); });
